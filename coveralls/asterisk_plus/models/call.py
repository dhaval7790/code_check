...
class Call(models.Model):
    ...
    def register_call(self):
        ...
        if self.answered_user:
            message += ' answered by {}'.format(self.answered_user.name)
        elif self.calling_user:
            message += ' from {}'.format(self.calling_user.name)
        elif self.called_users:
            # Missed call notification
            sub_register_call(
                self,
                subject=_('Missed call'),
                body=_('From {} {}'.format(
                    getattr(self.ref or self.partner or None, 'display_name', ''),
                    self.calling_number)),
                partner_ids=self.called_users.mapped('partner_id').mapped('id'))
            # Append dialed users to message for further register
            message += ' dialed: {}'.format(
                ', '.join(self.called_users.mapped('name')))
        if self.duration:
            message += ', duration: {}'.format(self.duration_human)
        # Register call at partner or reference object
        if self.partner and self.model != 'res.partner':
            sub_register_call(self.partner, body=message)
        if self.ref:
            sub_register_call(self.ref, body=message)
